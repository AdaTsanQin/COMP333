...
export default function OrderDetailsScreen({ navigation, route }) {
  ...
  const submit = async () => {
    ...
    if (!sid) {
      Alert.alert('Error', 'Session ID missing, please log in again.');
      return;
    }
    ...
    try {
      const r = await fetch(`${BASE_URL}/create_requests.php?PHPSESSID=${sid}`, {
        method: 'POST',
        headers: {
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {cart.map((it, i) => (
        <Text key={i} style={styles.itemTxt}>
          • {it.description}
          {it.items?.[0]?.price?.regular ? ` - $${it.items[0].price.regular}` : ''}
        </Text>
      ))}
...
const styles = StyleSheet.create({
  ...
  speedTxt: { color: '#333' },
  speedTxtSel: { color: '#fff', fontWeight: '700' },
  map: {
    width: '100%',
    height: 220,
    borderRadius: 6,
