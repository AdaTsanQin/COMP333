...
export default function AcceptOrderScreen({ route, navigation }) {
  ...
  const fetchOrders = async (sid = sessionID) => {
    if (!sid) return;
    try {
      const r = await fetch(`${BASE_URL}/accept_order.php?PHPSESSID=${sid}`, {
        credentials: 'include',
        headers: {
  ...
  const accept = async (id) => {
    try {
      const r = await fetch(`${BASE_URL}/accept_order.php?PHPSESSID=${sessionID}`, {
        method: 'PUT',
        credentials: 'include',
        headers: {
      ...
      Alert.alert('Error', 'Accept net err');
    }
  };
  ...
  const backendDrop = async (id) => {
    try {
      ...
      Alert.alert('Error', 'Drop net err');
    }
  };
